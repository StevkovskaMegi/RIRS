...
function App() {
  const [user, setUser] = useState(null);
  const navigate = useNavigate(); // Make sure this is inside the Router context

  useEffect(() => {
    const token = localStorage.getItem('token');

    if (token) {
      // Simulating token validation and setting user
      const decodedUser = { role: 'admin' }; // For testing, assume admin
      setUser(decodedUser);
